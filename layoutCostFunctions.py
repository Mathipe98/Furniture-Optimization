...
def findPathPoly(sourceP, targetP, objList, layoutPoly):
    ...
    for n in range(len(sourceP)):
        ...
        dDist = np.sqrt(
            (targetP[n][0] - sourceP[n][0]) ** 2 + (targetP[n][1] - sourceP[n][1]) ** 2
        )
...
def calcLayoutClearance(objList: list[list[tuple[float, float]]], layoutPoly, entList):
    """
...
def calcLayoutCirculation(
    objList: list[list[tuple[float, float]]], srcList: list[list], tgtList: list[list]
):
    """
    ...
    for i in range(nObj - 1):
        for j in range(i + 1, nObj):
            gSum += (
                ...
                * npla.norm(np.array(objPos[i]) - np.array(objPos[j]))
            )
